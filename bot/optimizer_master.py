...
def perturb(p: OptParams, rng: random.Random) -> OptParams:
    ...
    opts = {
        "coin": MASTER_COINS, "direction": MASTER_DIRECTIONS,
...
def main():
    ...
    coin_results: Dict[str, list] = {c: [] for c in MASTER_COINS}
    seen_keys: set = set()
    ...
    total_dl = len(MASTER_COINS) * len(TIMEFRAMES)
    print(f"[{ts()}] Descargando velas: {len(MASTER_COINS)} monedas × {len(TIMEFRAMES)} TF…")
    ...
    with ThreadPoolExecutor(max_workers=min(total_dl, 20)) as ex:
        futures = {ex.submit(_dl_task, coin, tf, cache_dir, args.days): (coin, tf)
                   for tf in TIMEFRAMES for coin in MASTER_COINS}
