#!/usr/bin/env python3
"""
AlphaChainBots — Bot de Liquidaciones WHALE
Solo opera en zonas donde liquidaciones superan $100M.
Pocas señales pero movimientos potencialmente grandes. Trailing stop 1%. x5 leverage.
"""
...
COINS           = ["BTC", "ETH"]   # solo las más líquidas para zonas >$100M
LEVERAGE        = 5
RISK_PER_TRADE  = 0.02
TRAILING_PCT    = 0.010
MIN_LIQ_USD     = 100_000_000      # $100M
MAX_DIST_PCT    = 0.030
LOOP_SLEEP      = 300
...
def get_liq_zones(coin,price):
    # Synthetic whale-scale zones
    rng=random.Random(int(price/100) ^ hash(coin) & 0xFFFF)
    scale={"BTC":1e9,"ETH":2e8}.get(coin,5e7); zones=[]
    for lev in [10,20,50,100]:
        d=1.0/lev*0.85
        for sign,zt in [(-1,"long"),(1,"short")]:
            amt=scale*rng.uniform(0.3,1.8)/(lev/20)
            if amt>=MIN_LIQ_USD*0.5:
                zones.append({"price":round(price*(1+sign*d*rng.uniform(0.85,1.15)),4),
                              "liq_usd":round(amt,0),"type":zt})
    return zones
...
class Bot:
    ...
    def run(self):
        log.info(" AlphaChainBots LIQ·WHALE — zonas >$100M")
        while True:
            try:
                ...
                for coin in COINS:
                    ...
                    if coin in self.positions:
                        ...
                        if pos["ts"].triggered(price): log.info(f"[{coin}] CIERRE WHALE"); del self.positions[coin]
                        continue
                    zones=get_liq_zones(coin,price)
                    candidates=[z for z in zones if z["liq_usd"]>=MIN_LIQ_USD and abs((z["price"]-price)/price)<=MAX_DIST_PCT]
                    if not candidates: continue
                    biggest=max(candidates,key=lambda z:z["liq_usd"])
                    sig="short" if biggest["price"]<price else "long"
                    ...
                    try:
                        ...
                        log.info(f"[{coin}] WHALE ENTRADA {sig.upper()} zona=${biggest['liq_usd']/1e6:.0f}M")
