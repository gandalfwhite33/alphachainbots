#!/usr/bin/env python3
"""
optimizer_v7.py — Optimizador de parámetros v7 para AlphaChainBots.
Extiende v6 con gestión avanzada de salida: tp_type (15 modos), tp_pct, tp_atr,
trailing_activation, trailing_progressive, atr_tp_adjust, partial_close,
partial_trigger, breakeven, time_exit, session_exit, weekend_exit, rr_min.
Añade trailing types: parabolic_sar, ma_trailing.

Uso:
    python optimizer_v7.py [--coin BTC] [--direction both] [--days 365]
                           [--samples 500000] [--out ./resultados_btc_v7]
                           [--workers N] [--resume]
"""
...
TP_PCT_OPTS      = [2, 5, 10, 15, 20, 30]          # % para fixed_pct
TP_ATR_OPTS      = [1.5, 2.0, 3.0, 5.0]            # multiplicador para atr_multiple
...
TIME_EXIT_OPTS   = ["none","5","10","20","50"]       # candles
...
def _pivot_levels(timestamps: np.ndarray, high: np.ndarray,
                  low: np.ndarray, close: np.ndarray,
                  weekly: bool = False) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Daily (or weekly) pivot P, R1, R2, S1, S2. Returns P, R1, R2, S1, S2."""
    ...
    for i in range(len(close)):
        ...
        if p != prev_p and prev_p >= 0:
            ...
            r2 = pivot + (ph - pl)
            s2 = pivot - (ph - pl)
            R1[i] = r1; S1[i] = s1; R2[i] = r2; S2[i] = s2; P[i] = pivot
            prev_p = p
        elif p == prev_p and i > 0:
            P[i] = P[i-1]; R1[i] = R1[i-1]; R2[i] = R2[i-1]
...
def _parabolic_sar(high: np.ndarray, low: np.ndarray,
    ...
    """Parabolic SAR. Returns SAR array aligned with close (same length)."""
    n = len(high)
...
def _worker_init(cache_pkl: str):
    ...
    # Pre-compute BTC trend direction per TF
    ...
    for tf, coins_dict in candles.items():
        ...
        for coin, arr in coins_dict.items():
            ...
            piv_p, piv_r1, piv_r2, piv_s1, piv_s2 = _pivot_levels(
                ts_col, highs, lows, closes, weekly=False)
            wpiv_p, wpiv_r1, wpiv_r2, wpiv_s1, wpiv_s2 = _pivot_levels(
...
def _calc_tp(tp_type: str, tp_pct: int, tp_atr_mult: float,
    ...
    """Calcula el nivel de TP según tp_type en la barra de entrada."""
    if tp_type == "none":
        return None
    ...
    elif tp_type == "pivot_s1":
        # For long: nearest S1 above entry acts as intermediate TP? Use R1 for longs.
        # "pivot_s1" = specifically S1, which for shorts is the target
        s1 = ind["piv_s1"][i]
        if not is_long and s1 == s1 and s1 < entry: return s1
        r1 = ind["piv_r1"][i]
        if is_long  and r1 == r1 and r1 > entry: return r1
...
def _calc_partial_trigger(partial_trigger: str, signal: str,
                          entry: float, stop: float, av: float) -> float:
    """Precio al que activar el primer parcial."""
    risk_dist = abs(entry - stop)
    is_long   = (signal == "long")
    if partial_trigger == "1atr":
        dist = av
    elif partial_trigger == "2atr":
        dist = 2.0 * av
    elif partial_trigger == "5pct":
        dist = entry * 0.05
    elif partial_trigger == "10pct":
        dist = entry * 0.10
    elif partial_trigger == "fib_127":
        dist = 1.272 * risk_dist
    else:  # fib_161
        dist = 1.618 * risk_dist
    return (entry + dist) if is_long else (entry - dist)


def _calc_breakeven_trigger(breakeven: str, signal: str,
                             entry: float, av: float) -> Optional[float]:
    """Precio al que mover SL a breakeven."""
...
def _simulate_fast(params: OptParams, coins_ind: Dict[str, dict]) -> dict:
    ...
    # v7 params
    tp_type_p    = params.tp_type
    tp_pct_p     = params.tp_pct
    tp_atr_p     = params.tp_atr
    trail_act    = params.trailing_activation
    trail_prog   = params.trailing_progressive
    atr_tp_adj   = params.atr_tp_adjust
    part_close   = params.partial_close
    part_trig    = params.partial_trigger
    be_mode      = params.breakeven
    time_exit_p  = params.time_exit
    sess_exit    = params.session_exit
    wknd_exit    = params.weekend_exit
    rr_min_p     = params.rr_min

    rr_min_val   = float(rr_min_p) if rr_min_p != "none" else None
    time_exit_n  = int(time_exit_p) if time_exit_p != "none" else None
    fng_val      = _FEAR_GREED_IDX
    fund_val     = _FUNDING_RATE
    ...
    for coin, ind in coins_ind.items():
        ...
        for i in range(min_bars, N - 1):
            ...
            if position is not None:
                ...
                part_done = position["pdone"]
                frac      = position["frac"]
                trig1     = position["trig1"]
                trig2     = position["trig2"]
                entry_bar = position["bar"]
                trail_act_flag = position["trail_act"]
                av_pos    = atr14[i] if atr14[i] == atr14[i] else entry * 0.01

                # ── Weekend exit ──────────────────────────────────────────────
                if wknd_exit and dow >= 4 and hour >= 20:
                    exit_price = price; exit_reason = "weekend"
                    # Fall through to exit block
                    pf1x = maf[i]; ps1x = mas[i]
                    exit_now = True
                else:
                    exit_now = False
                    exit_price = price
                ...
                # ── Session exit ──────────────────────────────────────────────
                if not exit_now and sess_exit and prev_hour_in_session and not cur_hour_in_session:
                    exit_now = True; exit_price = price

                # ── Time exit ─────────────────────────────────────────────────
                if not exit_now and time_exit_n is not None:
                    if (i - entry_bar) >= time_exit_n:
                        exit_now = True; exit_price = price
                ...
                if not exit_now:
                    ...
                    if not trail_act_flag:
                        threshold = None
                        if trail_act == "1pct":
                            threshold = entry * 1.01 if direction == "long" else entry * 0.99
                        elif trail_act == "2pct":
                            threshold = entry * 1.02 if direction == "long" else entry * 0.98
                        elif trail_act == "5pct":
                            threshold = entry * 1.05 if direction == "long" else entry * 0.95
                        elif trail_act == "10pct":
                            threshold = entry * 1.10 if direction == "long" else entry * 0.90
                        if threshold is None:
                            trail_act_flag = True  # "none" = always active
                        elif direction == "long"  and price >= threshold:
                            trail_act_flag = True
                        elif direction == "short" and price <= threshold:
                            trail_act_flag = True
                    ...
                    if trail_act_flag:
                        ...
                        eff_trail = trailing
                        if trail_prog:
                            profit_pct = ((price - entry) / entry if direction == "long"
                        ...
                        if trail_type == "atr_dynamic":
                            ns = (price - 2.0 * av_pos) if direction == "long" else (price + 2.0 * av_pos)
                            ...
                            ps_v = psar[i]
                            ns   = ps_v if ps_v == ps_v else stop
                        elif trail_type == "ma_trailing":
                            maf_v = maf[i]
                            ns    = maf_v if maf_v == maf_v else stop
                        elif sl_type == "trailing":
                            ns = (price * (1.0 - eff_trail) if direction == "long"
                                  else price * (1.0 + eff_trail))
                        elif sl_type == "atr":
                            ns = (price - 2.0 * av_pos) if direction == "long" else (price + 2.0 * av_pos)
                        else:
                            ns = stop
                        ...
                        if direction == "long"  and ns > stop: stop = ns; position["stop"] = stop
                        if direction == "short" and ns < stop: stop = ns; position["stop"] = stop

                    # ── Breakeven ─────────────────────────────────────────────
                    if not be_done and be_mode != "none":
                        be_trig = position.get("be_trig")
                        if be_trig is not None:
                            if direction == "long"  and price >= be_trig:
                                if entry > stop: stop = entry; position["stop"] = stop
                                position["be_done"] = True; be_done = True
                            elif direction == "short" and price <= be_trig:
                                if entry < stop: stop = entry; position["stop"] = stop
                                position["be_done"] = True; be_done = True
                    ...
                    if part_close != "none" and part_done < (1 if part_close == "2level" else 2):
                        ...
                        if trig is not None:
                            triggered = ((direction == "long"  and price >= trig) or
                                         (direction == "short" and price <= trig))
                            if triggered:
                                # Book partial PnL
                                ...
                                if pos_siz == "kelly" and kel_n >= 5:
                                    wr_k = kel_wins / kel_n
                                    rr_k = ((kel_gross_w / kel_wins) /
                                            (kel_gross_l / max(kel_n - kel_wins, 1))
                                            if kel_wins > 0 and kel_n > kel_wins else 1.0)
                                    f_s  = (wr_k * rr_k - (1.0 - wr_k)) / rr_k if rr_k > 0 else risk_lev
                                    eff_risk_p = max(0.005, min(f_s, 0.25))
                                size_p = coin_equity * eff_risk_p / entry * part_frac * frac
                                pnl_p  = (size_p * (trig - entry) if direction == "long"
                                          else size_p * (entry - trig))
                                ...
                                # Move stop to breakeven on first partial
                                if stop < entry and direction == "long": stop = entry; position["stop"] = stop
                                if stop > entry and direction == "short": stop = entry; position["stop"] = stop
                ...
                if exit_now:
                    if pos_siz == "kelly" and kel_n >= 5:
                        ...
                        eff_risk = (params.risk_pct * coin_equity) / (av2 * entry) if entry > 0 else risk_lev
                    ...
                    pnl  = (size * (exit_price - entry) if direction == "long"
                            else size * (entry - exit_price))
            ...
            if fng_filt != "none" and fng_val is not None:
                if fng_filt == "extreme_fear":
                    if signal == "short": continue
                elif fng_filt == "extreme_greed":
                    if signal == "long":  continue
                elif fng_filt == "both":
                    if fng_val >= 25 and fng_val <= 75: continue
            ...
            # ── Calcular TP con tp_type ───────────────────────────────────────
            tp = _calc_tp(tp_type_p, tp_pct_p, tp_atr_p,
                          signal, entry, stop, av_now, ind, i)

            # ── ATR TP adjustment ─────────────────────────────────────────────
            if tp is not None and atr_tp_adj != "none" and atr_mean > 0:
                ratio = av_now / atr_mean
                adj_factor = 1.0
                if atr_tp_adj in ("reduce_high", "both") and ratio > 1.5:
                    adj_factor = 0.80
                elif atr_tp_adj in ("extend_low", "both") and ratio < 0.7:
                    adj_factor = 1.20
                if adj_factor != 1.0:
                    tp_dist = abs(tp - entry) * adj_factor
                    tp = (entry + tp_dist) if signal == "long" else (entry - tp_dist)

            # ── RR minimum filter ─────────────────────────────────────────────
            if rr_min_val is not None and tp is not None:
                risk_d  = abs(entry - stop)
                reward_d = abs(tp - entry)
                if risk_d > 0 and reward_d / risk_d < rr_min_val:
                    continue  # skip trade

            # ── Partial trigger prices ────────────────────────────────────────
            ...
            # ── Trailing activation flag at entry ─────────────────────────────
            trail_act_at_entry = (trail_act == "none")  # if "none", immediately active
    ...
    return {
        ...
        "partial_exits": total_partial_exits,
    }
...
def save_json(out_dir: Path, global_top: List[dict], tf_tops: Dict[str, List[dict]],
    ...
    out_path = out_dir / f"top_params_{COIN_ID}_v7.json"
...
def save_excel(out_dir: Path, global_top: List[dict], tf_tops: Dict[str, List[dict]],
    ...
    out_path = out_dir / f"resultados_optimizacion_{COIN_ID}_v7.xlsx"
    try:
        wb.save(out_path)
...
def save_pdf(out_dir: Path, global_top: List[dict], tf_tops: Dict[str, List[dict]],
    ...
    out_path = out_dir / f"informe_optimizacion_{COIN_ID}_v7.pdf"
    try:
        with PdfPages(out_path) as pdf:
            fig, ax = plt.subplots(figsize=(11.69, 8.27))
            ax.axis("off")
            ax.text(0.5, 0.88, "AlphaChainBots", ha="center", fontsize=30, color=ACC, weight="bold")
            ax.text(0.5, 0.78, f"Informe de Optimizacion v7 — {COIN_ID.upper()} {DIRECTION.upper()}", ha="center", fontsize=16, color=FG)
            ...
            fig.suptitle("Resumen por Timeframe v7", fontsize=13, color=ACC)
            tf_names  = TIMEFRAMES
            ...
            for ax_idx, fld in enumerate(["tp_type", "partial_close", "breakeven"]):
                cnts: Dict[str, int] = {}
...
def print_summary(global_top: List[dict], tf_tops: Dict[str, List[dict]],
    ...
    sep = "=" * 80
    print(f"\n{sep}")
    print(f"[{ts()}]  OPTIMIZACION v7 COMPLETADA — {COIN_ID.upper()} {DIRECTION.upper()}")
    print(sep)
    print(f"  Combinaciones: {total_tried:,}  |  Duracion: {duration_sec/60:.1f} min")
    print(f"\n  TOP 5 GLOBAL:")
    print(f"  {'#':>2}  {'TF':>4}  {'MA':>12}  {'Lev':>4}  {'TP':>18}  "
          f"{'PnL$':>10}  {'WR%':>5}  {'DD%':>5}  {'Sharpe':>7}")
    print(f"  {'-'*80}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="AlphaChainBots Optimizer v7")
    ...
    all_coins_needed = list(set([coin_id, "BTC"]))
    ...
    SEG_SIZE   = 1_500   # slightly smaller due to richer position logic
