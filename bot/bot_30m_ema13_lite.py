#!/usr/bin/env python3
"""
AlphaChainBots — Hyperliquid Trading Bot (30M · EMA 13/34 · LITE)
Entrada : EMA 13/34 crossover + Volume 1.1x (sin Fibonacci ni S/R obligatorio)
...
def fetch_candles(info: HLInfo, coin: str, interval: str = INTERVAL,
    ...
    ms       = INTERVAL_MS.get(interval, INTERVAL_MS["30m"])
...
class Bot:
    ...
    def run(self) -> None:
        log.info("=" * 60)
        log.info(f" AlphaChainBots 30M·EMA13/34·LITE — {'TESTNET' if TESTNET else 'MAINNET'}")
        log.info(f" EMA {EMA_FAST}/{EMA_SLOW} | Trailing {TRAILING_PCT*100:.1f}%"
                 f" | RSI {RSI_OS}/{RSI_OB} | {LEVERAGE}x | LITE")
        log.info("=" * 60)
        self.coins = get_top_coins(self.info)
        while True:
            try:
                log.info(f"\n{'='*50}")
                log.info(f" Ciclo 30M: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
                log.info(f"{'='*50}")
                self.manage_positions()
                self.scan_entries()
                log.info(f"\nPosiciones: {list(self.positions.keys()) or 'ninguna'}")
                log.info(f"Próximo ciclo en 30m...\n")
