#!/usr/bin/env python3
"""
optimizer_v4.py — Optimizador de parámetros v4 para AlphaChainBots (solo BTC)
Igual que v3 pero analiza únicamente BTCUSDT para mayor profundidad de muestras.

Uso:
    python optimizer_v4.py [--days 365] [--samples 500000] [--out ./resultados_btc]
                           [--workers N] [--resume]
"""
...
def save_excel(out_dir: Path, global_top: List[dict], tf_tops: Dict[str, List[dict]],
    ...
    ws8.append(["optimizer_v4 analiza únicamente BTC (BTCUSDT)."])
    ...
    out_path = out_dir / "resultados_optimizacion_btc.xlsx"
    try:
        wb.save(out_path)
...
def save_pdf(out_dir: Path, global_top: List[dict], tf_tops: Dict[str, List[dict]],
    ...
    out_path = out_dir / "informe_optimizacion_btc.pdf"
    try:
...
def save_json(out_dir: Path, global_top: List[dict], tf_tops: Dict[str, List[dict]],
    ...
    out_path = out_dir / "top_params_btc.json"
...
def print_summary(global_top: List[dict], tf_tops: Dict[str, List[dict]],
    ...
    print(f"    resultados_optimizacion_btc.xlsx")
    print(f"    informe_optimizacion_btc.pdf")
    print(f"    top_params_btc.json")
    print(f"    checkpoint_btc.json")
    print(f"{sep}\n")
...
def main():
    ...
    cache_dir  = out_dir / "cache_btc"
    cache_dir.mkdir(parents=True, exist_ok=True)
    ckpt_path  = out_dir / "checkpoint_btc.json"
    ...
    # ── Cache para workers ────────────────────────────────────────────────────
    worker_pkl = out_dir / "_wcache_btc.pkl"
